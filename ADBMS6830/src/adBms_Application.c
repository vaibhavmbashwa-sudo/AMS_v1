...
void adBms6830_setgpo_69(uint8_t tIC, cell_asic *ic) {
	...
	float tV;
	for (int i = 0; i < tIC; i++) {
	    for (uint8_t value = 0x1; value <= 0xC; value++) {

					ic[i].tx_cfga.refon = PWR_UP;
					ic[i].tx_cfga.gpo = (((value << 5) & 0x3FF) | 0x21F);
					adBmsWakeupIc(tIC);
					adBmsWriteData(tIC, &ic[0], WRCFGB, Config, A);
					adBms6830_Adax(AUX_OPEN_WIRE_DETECTION, OPEN_WIRE_CURRENT_SOURCE, AUX_CH_TO_CONVERT);
					HAL_Delay(8);
					adBmsWakeupIc(tIC);
					adBmsReadData(tIC, &ic[0], RDAUXD, Aux, D);
					...
					int ind = (int)value;
					st->CELL_T[ind - 1]=v2Temp(tV);

					//IC temp
					adBmsWakeupIc(tIC);
					adBmsWriteData(tIC, &ic[0], WRCFGA, Config, A);
					adBmsWriteData(tIC, &ic[0], WRCFGB, Config, B);
					adBms6830_Adax(AUX_OPEN_WIRE_DETECTION, OPEN_WIRE_CURRENT_SOURCE, AUX_ALL);
					adBmsReadData(tIC, &ic[0], RDSTATA, Status, A);
