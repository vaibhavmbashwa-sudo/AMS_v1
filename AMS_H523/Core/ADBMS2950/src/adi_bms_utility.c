...
*/
const uint16_t ad29Crc15Table[256] =
{
...
static const uint16_t ad29crc10Table[256] =
    {
...
)
{
  ...
  for (uint8_t i = 0; i<len; i++) /*!< loops for each byte in data array */
  {
    ...
    remainder = ((remainder<<8u)^ad29Crc15Table[addr]);
  }
...
*******************************************************************************
*/
uint16_t ad29pec10_calc(bool rx_cmd, int len, uint8_t *data)
{
...
#endif
  ad29BmsCsLow();
  ad29spiWriteBytes(&cmd[0], 4);
  ad29BmsCsHigh();
  
  if(tx_cmd ==  SRSTs)
  {
    ad29Delay_us(RegulatorStartupTime_us);
  }  
}
...
#endif
    ad29BmsCsLow();
    ad29spiWriteReadBytes(&cmd[0], &data[0], RX_BUFFER);                 /*!< Read the configuration data of all ICs on the daisy chain into readdata array */
    ad29BmsCsHigh();
...
)
{
  ...
  else
  {
    ...
    ad29BmsCsLow();
    ad29spiWriteBytes(&cmd[0], CMD_LEN);
    ad29BmsCsHigh();
  }
  free(cmd);
}
...
//    printMsg("Failed to allocate memory");
  ...
  {
    ad29spiReadData(tIC, &cmd_arg[0], &read_buffer[0], &pec_error[0], &cmd_count[0], regData_size);
    switch (type)
    {
    case Config:
...
//    printMsg("Failed to allocate write_buffer array memory");
  ...
  adBmsWakeupIc(tIC);
#endif
  ad29spiWriteData(tIC, cmd_arg, &write_buffer[0]);
  free(write_buffer);
}
...
uint32_t ad29BmsPollAdc(uint8_t tIC, cell_asic_29 *ic, uint8_t tx_cmd[2])
{
  ...
  ad29startTimer();
  do{
    ad29spiReadBytes(&read_data, 1);
  }while(!(read_data == SDO_Line));
  pladc_count = ad29getTimCount();
  ad29BmsCsHigh();
  ad29stopTimer();
  return(pladc_count);
}
