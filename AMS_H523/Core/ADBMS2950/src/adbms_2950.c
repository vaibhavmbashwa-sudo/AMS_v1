...
VCH   VOLTAGE_MEASUREMENTs     = SM_V1;
RD    REDUNDANT_MEASUREMENTs   = RD_OFF;
...
const uint16_t MEASUREMENT_LOOP_TIMES  = 10;     /* milliseconds(mS)*/
...
void adi2950_init_config(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsWriteData(tIC, &ic[0], WRCFGAs, Config, A);
  // Delay for OCEN toggle
  ad29Delay_us(OC_delay_8cycle_us);
  ad29BmsWriteData(tIC, &ic[0], WRCFGAs, Config, B);
  // Delay for OCEN toggle
  ad29Delay_us(OC_delay_8cycle_us);
}
...
/**
*******************************************************************************
* @brief Write and Read Configuration Register A and B
*******************************************************************************
*/
void adi2950_write_read_config(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  /* Write Config Data */
  ad29BmsWriteData(tIC, &ic[0], WRCFGBs, Config, B);
  // Delay for OCEN toggle
  ad29Delay_us(OC_delay_8cycle_us);
  /* Read Config Data */
  ad29BmsReadData(tIC, &ic[0], RDCFGBs, Config, B);
  ...
  /* Write Config Data */
  ad29BmsWriteData(tIC, &ic[0], WRCOMMs, Comm, NONE);
  // Delay for OCEN toggle
  ad29Delay_us(OC_delay_8cycle_us);
  /* Read Config Data */
  ad29BmsReadData(tIC, &ic[0], RDCOMMs, Comm, NONE);

  ad29printWriteConfig(tIC, &ic[0], Config, ALL_GRP);
  ad29printReadConfig(tIC, &ic[0], Config, ALL_GRP);
#if 0
...
void adi2950_start_adi1_single_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  // Send ADI1 command with REDUNDANT_MEASUREMENTs off and SingleShot and start timer for first conversion
  adBms2950_Adi1(tIC, &ic[0], RD_OFF, 12, &command);
  ad29Delay_ms(ADI1_delay_ms);
  // Wait for 5msec or send PLADC command to poll for the data.
//#ifdef POLLING
//  ic[0].pladc_count = ad29BmsPollAdc(tIC, &ic[0], PLADCs);
//#else
//  ad29Delay_ms(Polling_ad29Delay_ms);
//#endif

//  ad29printMsg("Adi1 conversion completed");
//  ad29printPollAdcConvTime(ic[0].pladc_count);
}
...
void adi2950_start_adi1_continuous_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  // Send ADI1 command with REDUNDANT_MEASUREMENTs_ON for continuous mode without OW_WIRE_DETECTION
  adBms2950_Adi1(tIC, &ic[0], RD_ON, 4, &command);
  ad29Delay_ms(ADI1_delay_ms);
  // ad29Delay_ms(ADI1_ad29Delay_ms); /* As per data sheet current register update rate(1ms) min  & average current register update rate (8ms) max*/
  ...
  for (loop_counter=0; loop_counter < ITER_LOOP_MEASUREMENT_COUNT; loop_counter++)
  {
    ...
    do {
      ad29BmsRawWriteRead(tIC, &RFLAG[0], &data[index], size);
...
void adi2950_start_adi2_single_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printMsg("Adi2 single conversion completed");
  ad29printPollAdcConvTime(ic[0].pladc_count);
}
...
void adi2950_start_adi2_continuous_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29Delay_ms(8); /* As per data sheet current register update rate(1ms) min  & average current register update rate (8ms) max */
  ...
  adBms2950_Adi2(tIC, &ic[0], OPT0_SS, &command);
  ad29printMsg("Adi2 continuous conversion completed");
}
...
void adi2950_start_adi1_redundant_single_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  // Send ADI1 command with REDUNDANT_MEASUREMENTs ON and SingleShot and start timer for first conversion
  adBms2950_Adi1(tIC, &ic[0], RD_ON, OPT0_SS, &command);
  ad29Delay_ms(ADI1_delay_ms);
  // Wait for 5msec or send PLADC command to poll for the data.
#ifdef POLLING
  ...
  ad29printMsg("Adi1 redundant single conversion completed");
  ad29printPollAdcConvTime(ic[0].pladc_count);
}
...
void adi2950_start_adi1_redundant_continuous_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printMsg("Adi1 redundant continuous conversion completed");
}
...
void adi2950_read_cr_vbat_ivbat_registers(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsReadData(tIC, &ic[0], RDI, Cr, NONE);        /* Current Register Group */
//  ad29BmsReadData(tIC, &ic[0], RDVB, Vbat, NONE);   /* Battery Voltage Group*/
  ad29BmsReadData(tIC, &ic[0], RDIVB1, Ivbat, NONE); /* Current and Battery Voltage Group */

  ad29printCr(tIC, &ic[0]);
  ad29spiSendCmd(tIC, &ic[0], CLRI);
//  ad29printVbat(tIC, &ic[0]);
//  ad29printIvbat(tIC, &ic[0]);
//  TS_Current	=  getCurrent((uint32_t)&ic[0].ivbat.i1);
}
...
void adi2950_read_ocr_register(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsWriteData(tIC, &ic[0], WRCFGAs, Config, A);
  ad29BmsReadData(tIC, &ic[0], RDOC, Oc, NONE); /* Read OCxADC and REFADC results */
  ad29printOc(tIC, &ic[0]);
}
...
void adi2950_read_acccr_accvbat_accivbat_registers(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsReadData(tIC, &ic[0], RDIACC, AccCr, NONE);
  ad29BmsReadData(tIC, &ic[0], RDVBACC, AccVbat, NONE);
  ad29BmsReadData(tIC, &ic[0], RDIVB1ACC, AccIvbat, NONE);

  ad29printAccCr(tIC, &ic[0]);
  ad29printAccVbat(tIC, &ic[0]);
  ad29printAccIVbat(tIC, &ic[0]);
}
...
void adi2950_all_current_battery_voltage_registers(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsReadData(tIC, &ic[0], RDI, Cr, NONE);           //Current Register Group
  ad29BmsReadData(tIC, &ic[0], RDVB, Vbat, NONE);      //Battery Voltage register group
  ad29BmsReadData(tIC, &ic[0], RDIVB1, Ivbat, NONE);    //Current and Battery votage register group

  ad29BmsReadData(tIC, &ic[0], RDIACC, AccCr, NONE);      //Accumulated Current Register group
  ad29BmsReadData(tIC, &ic[0], RDVBACC, AccVbat, NONE);   //Accumulated battery votage group
  ad29BmsReadData(tIC, &ic[0], RDIVB1ACC, AccIvbat, NONE); //Accumulated battery voltage and current register group

  ad29BmsReadData(tIC, &ic[0], RDOC, Oc, NONE);         // Overcurrent ADC register group
  ...
  ic[0].pladc_count = ad29BmsPollAdc(tIC, &ic[0], PLV);
#else
  ad29Delay_ms(Polling_ad29Delay_ms);
#endif
#if 1
  // V1ADC register group
  ad29BmsReadData(tIC, &ic[0], RDV1A, GPV1, A);
  ad29BmsReadData(tIC, &ic[0], RDV1B, GPV1, B);
  ad29BmsReadData(tIC, &ic[0], RDV1C, GPV1, C);
  ad29BmsReadData(tIC, &ic[0], RDV1D, GPV1, D);
  ad29printVoltages(tIC, &ic[0], GPV1);
#else
  ad29BmsReadData(tIC, &ic[0], RDALLV, Rdallv, ALL_GRP);
  printRdallv(tIC, &ic[0]);
#endif
#if 1
  // V2ADC register group
  ad29BmsReadData(tIC, &ic[0], RDV2A, GPV2, A);
  ad29BmsReadData(tIC, &ic[0], RDV2B, GPV2, B);
  ad29BmsReadData(tIC, &ic[0], RDV2C, GPV2, C);
  ad29BmsReadData(tIC, &ic[0], RDV2D, GPV2, D);
  ad29BmsReadData(tIC, &ic[0], RDV2E, GPV2, E);
  ad29printVoltages(tIC, &ic[0], GPV2);
#else
  ad29BmsReadData(tIC, &ic[0], RDALLR, Rdallr, ALL_GRP);
  printRdallr(tIC, &ic[0]);
#endif

  // ADX command for Aux voltage conversion and start timer for first conversion
  ad29spiSendCmd(tIC, &ic[0], sADX);
#ifdef POLLING
  // PLX to poll on the conversion
  ic[0].pladc_count = ad29BmsPollAdc(tIC, &ic[0], PLX);
  ...
  ad29BmsReadData(tIC, &ic[0], RDXA, Aux, A);
  ad29BmsReadData(tIC, &ic[0], RDXB, Aux, B);
  ad29BmsReadData(tIC, &ic[0], RDXC, Aux, C);
  ad29printAux(tIC, &ic[0], Aux, A);
  ad29printAux(tIC, &ic[0], Aux, B);
  ad29printAux(tIC, &ic[0], Aux, C);
#else
  ad29BmsReadData(tIC, &ic[0], RDALLX, Rdallx, ALL_GRP);
...
void adi2950_start_adv_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29Delay_ms(Polling_ad29Delay_ms);
#endif
  ad29printMsg("Adv conversion completed");
  ad29printPollAdcConvTime(ic[0].pladc_count);
}
...
void adi2950_read_vrx_registers(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printAux(tIC, &ic[0], Aux, ALL_GRP);
}
...
*******************************************************************************
*/
// replaced by ad29spiSendCmd(tIC, &ic[0], sADX);
#if 0
void adi2950_start_adaux_measurment(uint8_t tIC)
{
  ad29BmsWakeupIc(tIC);
  adBms2950_Adaux(AUX_CH_TO_CONVERT);
  pladc_count = ad29BmsPollAdc(PLADC);
  ad29printMsg("Adaux conversion completed");
  ad29printPollAdcConvTime(pladc_count);
}
#endif
...
void adi2950_read_adaux_measurment(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  //ad29BmsReadData(tIC, &ic[0], RDSTATA, Status, A); /*!< Status A */
  ad29BmsReadData(tIC, &ic[0], RDXA, Status, A); /*!< Status A */
  ad29printStatus(tIC, &ic[0], Status, A);
}
...
void adi2950_read_status_register(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsReadData(tIC, &ic[0], RDSTAT, Status, E); /*!< Status E */

  ad29printStatus(tIC, &ic[0], Status, C);
}
...
void adi2950_read_all_status_registers(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printStatus(tIC, &ic[0], Status, ALL_GRP);
  ad29printFlag(tIC, &ic[0], Flag);
}
...
void adi2950_read_device_sid(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printDeviceSID(tIC, &ic[0], SID);
}
...
void adi2950_soft_reset(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printMsg("Soft Reset Done");
  ad29Delay_ms(8); /*  After SRST cmd 8ms delay required*/
}
...
void adi2950_reset_cmd_count(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29spiSendCmd(tIC, &ic[0], SRSTs);
  //RSTCC
  ad29spiSendCmd(tIC, &ic[0], RSTCCs);
  for(int cic=0;cic< tIC;cic++)
  {
    ic[cic].cccrc.cal_cmd_cntr =0;
  }

  ad29printMsg("Command Counter Reset Done");
}
...
void adi2950_snap(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29spiSendCmd(tIC, &ic[0], SNAPs);
  ad29Delay_ms(SNAP_Delay); //need to check in runtime ???

  ad29printMsg("Snap Done");
}
...
void adi2950_unsnap(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29spiSendCmd(tIC, &ic[0], UNSNAPs);
  ad29Delay_ms(SNAP_Delay);

  ad29printMsg("Unsnap Done");
}
...
{
  int option, gpo;
  ad29printMsg("Please Enter: \n1:Set\n2:Reset\n");
  ad29readUserInupt(&option);
  ...
  ad29BmsReadData(tIC, &ic[0], RDCFGAs, Config, A);
  ad29BmsReadData(tIC, &ic[0], RDSTAT, Status, E); /*!< Read status E for gpo pin status*/
  ad29printWriteConfig(tIC, &ic[0], Config, A);
  ad29printReadConfig(tIC, &ic[0], Config, A);
  ad29printStatus(tIC, &ic[0], Status, E);
}
...
void adi2950_gpio_spi_write_to_slave(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29BmsWriteData(tIC, &ic[0], WRCFGAs, Config, A);
  ad29BmsReadData(tIC, &ic[0], RDCFGAs, Config, A);
  ad29BmsReadData(tIC, &ic[0], RDSTAT, Status, E); /*!< Read Status E for gpio status*/

  ad29BmsWriteData(tIC, &ic[0], WRCOMMs, Comm, NONE);
  adBms2950_Stcomm(tIC, &ic[0]);
  ad29BmsReadData(tIC, &ic[0], RDCOMMs, Comm, NONE);

  ad29printMsg("GPIO SPI Write to Slave Completed");
  ad29printWriteConfig(tIC, &ic[0], Config, A);
  ad29printReadConfig(tIC, &ic[0], Config, A);
  ad29printStatus(tIC, &ic[0], Status, E);
  ad29printWriteCommData(tIC, &ic[0], Comm);
  ad29printReadCommData(tIC, &ic[0], Comm);
}
...
void adi2950_gpio_spi_read_from_slave(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  for(uint8_t cic = 0; cic < tIC; cic++)
  {
    ...
    ic[cic].tx_cfgb.gpio3c = (GPIOxC)PULL_DOWN_OFF;
    ic[cic].tx_cfgb.gpio4c = (GPIOxC)PULL_DOWN_OFF;
    ad29BmsWriteData(tIC, &ic[cic], WRCFGBs, Config, B);
  ...
  ad29printMsg("GPIO SPI Read from Slave Completed");
  ad29printWriteConfig(tIC, &ic[0], Config, A);
  ad29printReadConfig(tIC, &ic[0], Config, A);
  ad29printStatus(tIC, &ic[0], Status, E);
  ad29printWriteCommData(tIC, &ic[0], Comm);
...
void adi2950_gpio_i2c_write_to_slave(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  for(uint8_t cic = 0; cic < tIC; cic++)
  {
    ...
    ic[cic].tx_cfgb.gpio3c = (GPIOxC)PULL_DOWN_OFF;
    ic[cic].tx_cfgb.gpio4c = (GPIOxC)PULL_DOWN_OFF;
    // ad29BmsWriteData(tIC, &ic[0], WRCFGB, Config, B);
    // // Send RDCFGB to read configuration registers
    // ad29BmsReadData(tIC, &ic[0], RDCFGB, Config, B);
  ...
  // ad29BmsWriteData(tIC, &ic[0], WRCFGA, Config, A);
  // ad29BmsReadData(tIC, &ic[0], RDCFGA, Config, A);
  // ad29BmsReadData(tIC, &ic[0], RDSTAT, Status, E); /*!< Read Status E for gpio status*/
  ...
  ad29printMsg("GPIO I2C Write data to Slave completed");
  ad29printWriteConfig(tIC, &ic[0], Config, A);
  ad29printReadConfig(tIC, &ic[0], Config, A);
  ad29printWriteConfig(tIC, &ic[0], Config, B);
  ad29printReadConfig(tIC, &ic[0], Config, B);
  ad29printStatus(tIC, &ic[0], Status, E);
  ad29printWriteCommData(tIC, &ic[0], Comm);
  ad29printReadCommData(tIC, &ic[0], Comm);
}
...
void adi2950_gpio_i2c_read_from_slave(uint8_t tIC, cell_asic_29 *ic)
{
  ...
  ad29printMsg("GPIO I2C Read data from Slave completed");
  ad29printReadCommData(tIC, &ic[0], Comm);
  ad29printStatus(tIC, &ic[0], Status, E);
}
